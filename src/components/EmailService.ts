 ...
 * @returns {Promise<void>}
 */
    static async sendVerificationEmail(to: string, token: string, name: string): Promise<void> {
 ...
 * @returns {Promise<void>}
 */
    static async sendSuccessfulRegistration(to: string, token: string, name: string): Promise<void> {
        const subject = 'Email Verification';
        // replace this url with the link to the email verification page of your front-end app
        const verificationEmailUrl = `http://${config.clientUrl}/verify-email?token=${token}`;
        const text = `Hi ${name},
     ...
     * @returns {Promise<void>}
     */
    static async sendAccountCreated(to: string, name: string): Promise<void>{
